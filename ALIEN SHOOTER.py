...
def main():
    ...
    pygame.display.set_caption('Alien Shooting')
...
# Prepare game objects
    ...
    # Detertmine when to move to next wave
        if aliensLeftThisWave <= 0:
            if betweenWaveCount > 0:
                ...
                if wave % 4 == 0:
                    speedUpText = font.render('LEVEL UP!', 1, BLUE)
    ...
    # Event Handling
        ...
        if isHiScore:
            hiScoreText = font.render('HIGH SCORE!', 1, BLUE)
            ...
            enterNameText = font.render('ENTER YOUR NAME:', 1, BLUE)
            enterNamePos = enterNameText.get_rect(midtop=scorePos.midbottom)
            nameText = font.render(name, 1, RED)
            ...
            gameOverPos = gameOverText.get_rect(
                center=screen.get_rect().center)
            scoreText = font.render('SCORE: {}'.format(score), 1, (255,255,0))
